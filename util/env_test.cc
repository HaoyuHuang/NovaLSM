...
namespace leveldb {
    ...
    TEST(EnvTest, ReadWrite) {
        ...
        while (data.size() < kDataSize) {
            int len = rnd.Skewed(
                    18);  // Up to 2^18 - 1, but typically much smaller
            std::string r;
        ...
        while (read_result.size() < data.size()) {
            int len = std::min<int>(rnd.Skewed(18),
                                    data.size() - read_result.size());
            scratch.resize(
                    std::max(len, 1));  // at least 1 so &scratch[0] is legal
            Slice read;
...
//        env_->Schedule(&Callback::Run, &callback4);
    ...
    TEST(EnvTest, TestOpenNonExistentFile) {
        ...
        Status status =
                env_->NewRandomAccessFile(non_existent_file,
                                          &random_access_file);
