...
// Helper for quickly generating random data.
    ...
    public:
        ...
        void Run() {
            ...
            while (benchmarks != nullptr) {
                ...
                if (name == Slice("fillseq")) {
                    ...
                    write_sync = true;
                    Write(write_sync, RANDOM, FRESH, num_ / 100,
                          FLAGS_value_size, 1);
                    WalCheckpoint(db_);
                    ...
                    Write(write_sync, RANDOM, FRESH, num_ / 1000, 100 * 1000,
                          1);
                    WalCheckpoint(db_);
                } else if (name == Slice("fillseq100K")) {
                    Write(write_sync, SEQUENTIAL, FRESH, num_ / 1000,
                          100 * 1000, 1);
                    WalCheckpoint(db_);
        ...
        void Open() {
            ...
            if (FLAGS_page_size != 1024) {
                ...
                snprintf(page_size, sizeof(page_size), "PRAGMA page_size = %d",
                         FLAGS_page_size);
                status = sqlite3_exec(db_, page_size, nullptr, nullptr,
                                      &err_msg);
                ExecErrorCheck(status, err_msg);
            }
            ...
            if (FLAGS_WAL_enabled) {
                ...
                status = sqlite3_exec(db_, WAL_stmt.c_str(), nullptr, nullptr,
                                      &err_msg);
                ExecErrorCheck(status, err_msg);
                status =
                        sqlite3_exec(db_, WAL_checkpoint.c_str(), nullptr,
                                     nullptr, &err_msg);
                ExecErrorCheck(status, err_msg);
            }
            ...
            for (int i = 0; i < stmt_array_length; i++) {
                status =
                        sqlite3_exec(db_, stmt_array[i].c_str(), nullptr,
                                     nullptr, &err_msg);
                ExecErrorCheck(status, err_msg);
            }
        }
        ...
        void Write(bool write_sync, Order order, DBState state, int num_entries,
            ...
            status = sqlite3_exec(db_, sync_stmt.c_str(), nullptr, nullptr,
                                  &err_msg);
            ExecErrorCheck(status, err_msg);
            ...
            for (int i = 0; i < num_entries; i += entries_per_batch) {
                ...
                for (int j = 0; j < entries_per_batch; j++) {
                    ...
                    const int k =
                            (order == SEQUENTIAL) ? i + j : (rand_.Next() %
                                                             num_entries);
                    char key[100];
                    snprintf(key, sizeof(key), "%016d", k);

                    // Bind KV values into replace_stmt
                    status = sqlite3_bind_blob(replace_stmt, 1, key, 16,
                                               SQLITE_STATIC);
                    ErrorCheck(status);
        ...
        void Read(Order order, int entries_per_batch) {
            ...
            for (int i = 0; i < reads_; i += entries_per_batch) {
                ...
                for (int j = 0; j < entries_per_batch; j++) {
                    // Create key value
                    char key[100];
                    int k = (order == SEQUENTIAL) ? i + j : (rand_.Next() %
                                                             reads_);
                    snprintf(key, sizeof(key), "%016d", k);

                    // Bind key value into read_stmt
                    status = sqlite3_bind_blob(read_stmt, 1, key, 16,
                                               SQLITE_STATIC);
                    ErrorCheck(status);
        ...
        void ReadSequential() {
            ...
            status = sqlite3_prepare_v2(db_, read_str.c_str(), -1, &pStmt,
                                        nullptr);
            ErrorCheck(status);
            for (int i = 0;
                 i < reads_ && SQLITE_ROW == sqlite3_step(pStmt); i++) {
