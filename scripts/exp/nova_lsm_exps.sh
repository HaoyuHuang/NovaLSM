...
# for dist in "zipfian"
# do
# bash /proj/bg-PG0/haoyu/latest/nova_lsm_subrange_backup.sh $recordcount $dryrun $number_of_ltcs $nservers $num_memtable_partitions $dist $num_sstable_replicas  >> lsm_backup_out
# done
# done
# done
# done

# bash /proj/bg-PG0/haoyu/latest/nova_lsm_subrange_replication.sh $recordcount $dryrun > stoc_scale_out
...
do
bash /proj/bg-PG0/haoyu/scripts/nova_lsm_subrange_backup.sh $recordcount $dryrun $number_of_ltcs $nservers $num_memtable_partitions $dist $num_sstable_replicas  >> lsm_backup_out 
done
done
done
