...
//            stats_[level].Add(stats);
        ...
        for (auto &it : pid_tasks) {
            ...
            MemTablePartition *p = active_memtables_[it.first];
            p->mutex.Lock();
            bool no_slots = p->available_slots.empty();
            for (auto &task : it.second) {
                RDMA_ASSERT(task.imm_slot < imms_.size());
                RDMA_ASSERT(imms_[task.imm_slot]);
                MemTable *imm = reinterpret_cast<MemTable *>(task.memtable);
                imms_[task.imm_slot] = nullptr;
                p->closed_memtable_ids.push_back(
                        imm->memtableid());
...
//            Log(options_.info_log,
//                "bg[%lu]: Level-0 table #%llu pid-%u mem-%u done",
//                bg_thread->thread_id(),
//                (unsigned long long) meta.number, task.memtable_partition_id,
//                imm->memtableid());
        return true;
...
//        return s;
    ...
    void DBImpl::MaybeScheduleCompaction(uint32_t thread_id,
        ...
        uint32_t i = bg_thread_id_seq_.fetch_add(1, std::memory_order_relaxed) %
                     bg_threads_.size();
        CompactionTask task = {};
        task.db = this;
        task.memtable = imm;
...
//        return internal_iter;
    ...
    Status DBImpl::Get(const ReadOptions &options, const Slice &key,
        ...
        for (int i = 0; i < active_memtables_.size(); i++) {
            active_memtables_[i]->mutex.Lock();
            MemTable *table = versions_->mid_table_mapping_[active_memtables_[i]->memtable->memtableid()].Ref(
                    nullptr);
            if (table) {
                mems.push_back(table);
            }
            for (int j = 0; j < active_memtables_[i]->imm_slots.size(); j++) {
                table = imms_[active_memtables_[i]->imm_slots[j]];
                if (table) {
                    table = versions_->mid_table_mapping_[table->memtableid()].Ref(
                            nullptr);
                    if (table) {
                        mems.push_back(table);
                    }
            ...
            active_memtables_[i]->mutex.Unlock();
        }

        // Look at the current version next. It must have included keys compacted.
        Version *current = nullptr;
        uint32_t vid = 0;
        while (current == nullptr) {
            vid = versions_->current_version_id();
...
//        }
    ...
    bool DBImpl::Write(const leveldb::WriteOptions &options,
        ...
        while (true) {
            ...
            next_imm_slot = -1;
            bool is_full = table->ApproximateMemoryUsage() >
                           options_.write_buffer_size;
            if (nova::NovaConfig::config->measure_recovery_duration) {
                is_full = table->nentries_ == 15732;
            }
            if (!is_full) {
                break;
            } else {
                ...
                if (next_imm_slot == -1) {
                    ...
                    table = new MemTable(internal_comparator_, memtable_id,
                                         db_profiler_, options_.mem_manager);
        ...
        for (auto writer : partition->writers) {
            LevelDBLogRecord record = {};
            record.key = writer->key;
            record.value = writer->value;
            record.sequence_number = writer->sequence;
        ...
        dc->set_dbid(dbid_);
        uint32_t dc_index = 0;
        switch (nova::NovaConfig::config->log_record_policy) {
            case nova::LogRecordPolicy::NONE:
                break;
            case nova::LogRecordPolicy::EXCLUSIVE_LOG_FILE:
                dc_index = memtable_id %
                           nova::NovaCCConfig::cc_config->dc_servers.size();
            case nova::LogRecordPolicy::SHARED_LOG_FILE: {
                ...
                break;
            case nova::LogRecordPolicy::IN_MEMORY_LOG:
                MemTableIdentifier memtable_identifier = {};
                memtable_identifier.cc_id = nova::NovaConfig::config->my_server_id;
                memtable_identifier.db_id = dbid_;
                memtable_identifier.memtable_id = memtable_id;
                dc->InitiateReplicateLogRecords(memtable_identifier,
                                                options.client_worker_id,
                                                log_records);
                ...
                for (const auto &close_id : closed_memtable_ids) {
                    ...
                    memtable_identifier.cc_id = nova::NovaConfig::config->my_server_id;
                    memtable_identifier.db_id = dbid_;
                    memtable_identifier.memtable_id = close_id;
                    dc->InitiateCloseLogFile(memtable_identifier);
                }
                break;
        }
        ...
        for (int i = 0; i < log_records.size(); i++) {
            table->Add(log_records[i].sequence_number, ValueType::kTypeValue,
                       log_records[i].key,
                       log_records[i].value);
            if (table_locator_ != nullptr) {
                table_locator_->Insert(log_records[i].key, options.hash,
                                       table->memtableid());
            }
        }
        ...
        *dbptr = nullptr;
        ...
        if (s.ok()) {
            ...
            if (s.ok()) {
                ...
                for (int i = 0; i < impl->imms_.size(); i++) {
                    impl->imms_[i] = nullptr;
                }
                ...
                for (int i = 0; i < options.num_memtable_partitions; i++) {
                    ...
                    MemTable *table = new MemTable(
                            ...
                            memtable_id,
                            impl->db_profiler_, options.mem_manager);
                    RDMA_ASSERT(memtable_id < MAX_LIVE_MEMTABLES);
                    impl->versions_->mid_table_mapping_[memtable_id].SetMemTable(
                            table);
                    impl->active_memtables_[i] = new DBImpl::MemTablePartition;
                    impl->active_memtables_[i]->memtable = table;
                    impl->active_memtables_[i]->partition_id = i;
                ...
                for (int i = 0; i < options.num_memtable_partitions; i++) {
                    for (int j = 0; j <
                                    impl->active_memtables_[i]->imm_slots.size(); j++) {
                        RDMA_ASSERT(slot_id ==
                                    impl->active_memtables_[i]->imm_slots[j]);
                        slot_id++;
                    }
