...
struct Stats {
    ...
    Stats diff(const Stats &other) {
        ...
        diff.nget_rdma_invalid = nget_rdma_invalid - other.nget_rdma_invalid;
...
public:
    ...
    void
    PostRDMAGETIndexRequest(int fd, char *key, uint64_t nkey, int home_server,
...
//    DataEntry data_entry;
    ...
    void UpdateEventFlags(int new_flags) {
        ...
        RDMA_ASSERT(
                event_assign(&event, worker->base, fd, new_flags, event_handler,
                             this) ==
                0) << fd;
