...
public:
    ...
    Request(char cmd, const string &key, const string &value) : cmd(cmd), key(key), value(value) {}
...
public:
    static Request parse_request(char *buf) {
        ...
        if (cmd == NOVA_REQ_GET) {
            ...
            RDMA_LOG(DEBUG) << "Received get request " << key;
            ...
            RDMA_LOG(DEBUG) << "Received put request " << key << " " << value;
            return Request(NOVA_REQ_SET, key, value);
        }
        RDMA_ASSERT(false) << "unknow request " << buf;
        return Request();
    }
        ...
        while (tmpbuf[len] != REQ_TERMINATER_CHAR) {
            if (tmpbuf[len] == REQ_BATCH_DELIMITER_CHAR) {
                ...
                if (cmd == NOVA_REQ_GET) {
                    ...
                    RDMA_ASSERT(false) << "unknown request " << buf;
                }
                tmpbuf += len + 1;
                len = 0;
                continue;
