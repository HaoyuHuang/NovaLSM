...
#ifndef RLIB_NOVA_MEMCACHED_HASHTABLE_H
#define RLIB_NOVA_MEMCACHED_HASHTABLE_H
...
public:
    ...
    ChainedHashTable(char *backing_mem, uint64_t index_size,
        ...
        nbuckets_ = static_cast<uint32_t>(main_bucket_mem_size / bucket_size());
        enable_eviction_ = enable_eviction;
        index_only_ = index_only;

        free_buckets_base_ = backing_mem + main_bucket_mem_size;
        nfree_buckets_ = static_cast<uint32_t>(free_indirect_bucket_mem_size /
                                               bucket_size());
        free_bucket_index_ = 0;
        is_full_.store(false);
        RDMA_LOG(INFO) << "Hash table index entry size " << IndexEntry::size()
                       << " bucket size "
                       << bucket_size();
        RDMA_LOG(INFO) << "Create " << nbuckets_ << " main buckets and "
        ...
        while (bucket) {
            ...
            for (uint32_t i = 0; i < nindex_entry_per_bucket_; i++) {
                ...
                if (index_entry.hash == hv) {
                    ...
                    if (update_time) {
                        index_entry.write_time(index_entry_buf, access_time);
                    }
    ...
    PutResult Delete(uint64_t hv, char *key, uint32_t nkey, bool check_key) {
        PutResult result;
    ...
    PutResult
    Put(const char *data_ptr, uint32_t slab_class_id, uint64_t hv, char *key,
        uint32_t nkey, uint64_t ndata_size, bool check_key) {
        if (index_only_) {
            RDMA_ASSERT(
                    data_ptr == nullptr && slab_class_id == 0 && !check_key);
        }
        // The main header is locked.
    ...
    PutResult Put(const IndexEntry &new_index_entry, char *key, uint32_t nkey,
                  bool check_key) {
        PutResult result{};
        result.success = false;
        uint64_t hv = new_index_entry.hash;
        uint32_t index = bucket_index(hv);
        RDMA_LOG(DEBUG) << "associate item " << hv << " to " << index;
        char *bucket = Bucket(index);
        ...
        while (bucket) {
            ...
            for (uint32_t i = 0; i < nindex_entry_per_bucket_; i++) {
                ...
                if (index_entry.type == IndexEntryType::EMPTY) {
                    ...
                    continue;
                } else if (index_entry.type == IndexEntryType::INDRECT_HEADER) {
                    bucket = (char *) index_entry.data_ptr;
                    has_indirect_header = true;
                    break;
                }
        ...
        if (!empty_index_entry_buf) {
            ...
            if (!is_full_.load()) {
                ...
                IndexEntry index_entry = IndexEntry::chars_to_indexitem(
                        index_entry_buf);
                RDMA_ASSERT(index_entry.type == 0);
                RDMA_LOG(DEBUG) << "Create indirect header for key " << hv;
...
#endif //RLIB_NOVA_MEMCACHED_HASHTABLE_H
