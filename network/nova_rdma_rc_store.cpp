...
//    RNicHandler *device = nullptr;
    ...
    if (!is_device_opened) {
        ...
        RDMA_ASSERT(
                NovaConfig::rdma_ctrl->register_memory(my_memory_id, cache_buf,
...
//    device = NovaConfig::rdma_ctrl->open_thread_local_device(idx);
    ...
    for (int peer_sid = 0; peer_sid < num_servers; peer_sid++) {
        ...
        QPIdx peer_rc_key = create_rc_idx(peer_sid, thread_id_, my_server_id);
//        uint64_t peer_memory_id = static_cast<uint64_t >(peer_sid) << 32 |
//                                  static_cast<uint64_t>(thread_id_);
        uint64_t peer_memory_id = static_cast<uint64_t >(peer_sid);
        RDMA_LOG(INFO) << "rdma-rc[" << thread_id_ << "]: get server memory id "
                       << peer_memory_id << " from "
                       << peer_store.ip << ":" << peer_store.port;
        MemoryAttr local_mr = NovaConfig::rdma_ctrl->get_local_mr(my_memory_id);
        ...
        while (QP::get_remote_mr(peer_store.ip, NovaConfig::config->rdma_port,
                                 peer_memory_id, &remote_mr) != SUCC) {
            usleep(CONN_SLEEP);
        }
        qp_[peer_sid]->bind_remote_mr(remote_mr);
        RDMA_LOG(INFO) << "rdma-rc[" << thread_id_ << "]: connect to server "
                       << peer_store.ip << ":" << peer_store.port;
        ...
        RDMA_LOG(INFO) << "rdma-rc[" << thread_id_ << "]: connected to server "
                       << peer_store.ip << ":" << peer_store.port;
    }
}

void
NovaRDMARCStore::PostRead(uint32_t size, int server_id, uint64_t local_offset,
    ...
    if (send_sge_index_[server_id] == doorbell_batch_size_) {
        ...
        RDMA_LOG(DEBUG) << "rdma-rc[" << thread_id_ << "]: " << "SQ: posting "
...
void NovaRDMARCStore::FlushPendingSends() {
    for (int peer_sid = 0;
        ...
        RDMA_LOG(DEBUG) << "flush pending reads " << send_sge_index_[peer_sid];
...
void NovaRDMARCStore::PollSQ(int peer_sid) {
    ...
    for (int i = 0; i < n; i++) {
        RDMA_ASSERT(wcs_[i].status == IBV_WC_SUCCESS)
            ...
            << wcs_[i].status << " str:" << ibv_wc_status_str(wcs_[i].status);
...
char *NovaRDMARCStore::GetSendBuf(int server_id) {
    int max_msg_size_ = NovaConfig::config->max_msg_size;
    return rdma_read_buf_[server_id] + pread_index_[server_id] * max_msg_size_;
}