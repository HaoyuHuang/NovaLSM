...
static uint64_t tab_hash(const char *key, size_t len) {
...
static uint64_t sbox_hash(char *key, size_t len) {
    // a large prime number
    uint32_t h = 4294967291U;
    while (len) {
        len--;
...
static uint64_t noop_hash(const char *key, size_t len) {
//    assert(len == sizeof(uint64_t));
//    (void) len;
    return *(uint64_t *) key;
}

static uint64_t mul_hash(const char *key, size_t len) {
//    assert(len == sizeof(uint64_t));
//    (void) len;
    // a large prime number
...
static uint64_t cityhash(const char *key, size_t len) {
...
struct SessionEntry {
    ...
    static SessionEntry chars_to_sessionitem(char *buf) {
        SessionEntry it{};
        char *tmp = buf;
        memcpy(&it.type, tmp, sizeof(uint8_t));
        tmp += sizeof(uint8_t);
        memcpy(&it.nkey, tmp, sizeof(uint32_t));
        return it;
    }

    static void sessionitem_to_chars(char *_base, const SessionEntry &entry) {
        char *buf = _base;
        memcpy(buf, &entry.type, sizeof(uint8_t));
        buf += sizeof(uint8_t);
        memcpy(buf, &entry.nkey, sizeof(uint32_t));
    }
};
...
struct DataEntry {
    ...
    static uint32_t sizeof_data_entry(uint32_t nkey, uint32_t nval) {
        return sizeof(uint8_t) + sizeof(uint32_t) * 2 + nint_to_str(nval) + 1 +
               1 +
               nkey + nval +
               sizeof(uint64_t);
    }
    ...
    static void
    dataitem_to_chars(char *buf, char *key, uint32_t nkey, char *val,
