...
public:
    ...
    static Fragment *home_fragment(uint64_t key) {
        if (config->partition_mode == NovaRDMAPartitionMode::HASH) {
            ...
            RDMA_ASSERT(
                    key <= config->fragments[config->nfragments - 1]->key_end);
            uint32_t l = 0;
    ...
    void ReadFragments(const std::string &path) {
        ...
        while (std::getline(file, line)) {
            ...
            RDMA_ASSERT(
                    (iss >> frag->key_start >> frag->key_end >> frag->server_id
                         >> frag->worker_id));
        ...
        RDMA_LOG(INFO) << "Configuration has a total of " << frags.size()
                       << " fragments.";
        for (int i = 0; i < nfragments; i++) {
            RDMA_LOG(DEBUG) << "frag[" << i << "]: " << fragments[i]->key_start
                            << "-" << fragments[i]->key_end
    ...
    void ComputeNumberOfBuckets() {
        ...
        nbuckets = static_cast<uint32_t>(main_bucket_mem_size / bucket_size());
    ...
    string to_string() {
        ...
        sprintf(output,
                ...
                enable_load_data, enable_rdma, cache_size_gb, index_size_mb);
