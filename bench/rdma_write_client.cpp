...
namespace nova {
    ...
    uint32_t RDMAWRITEClient::Initiate() {
        if (!buf_) {
            ...
            RDMA_LOG(DEBUG) << fmt::format("Allocate size {}", size);
    ...
    RDMAWRITEClient::ProcessRDMAWC(ibv_wc_opcode type, uint64_t wr_id,
        ...
        switch (type) {
            ...
            case IBV_WC_RDMA_WRITE: {
                ...
                rdma_store_->PostSend(sendbuf, 1, remote_server_id, req_id);
                processed = true;
            ...
            case IBV_WC_RECV_RDMA_WITH_IMM:
                ...
                RDMA_ASSERT(req_context_.find(req_id) != req_context_.end());
                RequestContext &ctx = req_context_[req_id];
