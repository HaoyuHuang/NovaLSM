...
DEFINE_string(servers, "localhost:11211", "A list of cc servers");
...
int main(int argc, char *argv[]) {
    ...
    RDMA_ASSERT(rdma_buf != NULL) << "Not enough memory";
...
//            uint32_t num_persist_workers
    ...
    // Address must match.
    RDMA_ASSERT((uint64_t) (rdma_worker_buf) == (uint64_t) (cache_buf));
    ...
    RDMA_LOG(INFO) << fmt::format("Total requests: {}", requests);
    return 0;
}
