        ...
        while (!async_cq_.empty()) {
            ...
            if (task.request_type ==
                ...
                if (!is_local_disk_bench_) {
                    ...
                    uint32_t msg_size = 1;
                    rdma_store_->PostSend(sendbuf, msg_size,
                                          task.remote_server_id,
                                          task.dc_req_id);
                }
...
//            rdma_store_->PollRQ();
    ...
    RDMAWRITEServerWorker::ProcessRDMAWC(ibv_wc_opcode type, uint64_t wr_id,
        ...
        switch (type) {
            ...
            case IBV_WC_RECV_RDMA_WITH_IMM:
                ...
                if (it != req_context_.end()) {
                    // Mark as written.
                    processed_by_server = true;
                    RDMA_ASSERT(it->second.local_buf[0] == '1');
                ...
                if (buf[0] == BenchRequestType::BENCH_ALLOCATE) {
                    ...
                    rdma_store_->PostSend(sendbuf, 9, remote_server_id, req_id);
                    processed_by_server = true;
        ...
        RDMA_ASSERT((processed_by_client || processed_by_server));
        RDMA_ASSERT(!(processed_by_client && processed_by_server));
    }
