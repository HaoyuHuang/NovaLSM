...
namespace nova {
            ...
            if (n <= 0) {
                if (errno == EWOULDBLOCK || errno == EAGAIN) {
                    ...
                    RDMA_LOG(WARNING) << "memstore[" << store->thread_id_ << "]: "
                                      << "write socket would block fd: " << fd << " "
                                      << strerror(errno);
    ...
    void event_handler(int fd, short which, void *arg) {
        ...
        if (conn->state == ConnState::READ) {
            ...
            if (state == COMPLETE) {
                if (worker->stats.nreqs % 99 == 0 && worker->stats.nreqs > 0) {
                    timeval now{};
                    gettimeofday(&now, nullptr);
                    auto elapsed = (now.tv_sec - worker->read_start.tv_sec) * 1000 * 1000 +
                                   (now.tv_usec - worker->read_start.tv_usec);
                    worker->stats.read_service_time += elapsed;
                }
    ...
    void write_socket_complete(int fd, Connection *conn) {
        ...
        if (worker->stats.nreqs % 99 == 0 && worker->stats.nreqs > 0) {
            timeval now{};
            gettimeofday(&now, nullptr);
            auto elapsed = (now.tv_sec - worker->start.tv_sec) * 1000 * 1000 +
                           (now.tv_usec - worker->start.tv_usec);
            auto write_elapsed =
                    (now.tv_sec - worker->write_start.tv_sec) * 1000 * 1000 +
                    (now.tv_usec - worker->write_start.tv_usec);
            worker->stats.service_time += elapsed;
    ...
    bool process_socket_get(int fd, Connection *conn, char *buf) {
        // Stats.
        NovaConnWorker *worker = (NovaConnWorker *) conn->worker;
        worker->stats.ngets++;
        uint64_t int_key = 0;
    ...
    bool process_socket_scan(int fd, Connection *conn, char *buf) {
        // Stats.
        NovaConnWorker *worker = (NovaConnWorker *) conn->worker;
        ...
        uint64_t intkey = 0;
        startkey = buf;
        int nkey = str_to_int(buf, &intkey) - 1;
        buf += nkey + 1;
        uint64_t nrecords;
        ...
        Fragment *frag = NovaConfig::home_fragment(hv);
        uint32_t pivot_db_id = frag->dbid;
        ...
        //        RDMA_LOG(INFO)
        //            << fmt::format("Scan fd: {} key:{} nkey:{} cardinality:{}
        //            size:{}",
    ...
    bool process_socket_put(int fd, Connection *conn, char *buf) {
        ...
        Fragment *frag = NovaConfig::home_fragment(hv);
        RDMA_LOG(DEBUG) << "memstore[" << worker->thread_id_ << "]: "
                        << " put fd:" << fd << ": key:" << key << " nkey:" << nkey
                        << " nval:" << nval;

        rocksdb::Slice dbkey(ckey, nkey);
        rocksdb::Slice dbval(val, nval);
    ...
    bool process_socket_drain_l0_sstable_request(int fd, Connection *conn) {
    ...
    bool process_socket_stats_request(int fd, Connection *conn) {
        ...
        for (auto db : worker->dbs_) {
            ...
            RDMA_ASSERT(db->GetIntProperty("rocksdb.num-running-compactions", &num_running_compactions));
            RDMA_ASSERT(db->GetIntProperty("rocksdb.num-running-flushes", &num_running_flushes));
            if (std::stoi(value) == 1 || num_running_compactions > 0 || num_running_flushes > 0) {
                needs_compaction = true;
            }
            RDMA_LOG(INFO)
                << fmt::format("Compaction pending:{}. Num running compaction:{}. Num running flushes:{}.", value,
                               num_running_compactions, num_running_flushes);
        }
    ...
    void stats_handler(int fd, short which, void *arg) {
        ...
        if (store->stats.nreqs > 0) {
            ...
            read_service_time = store->stats.read_service_time / store->stats.nreqs;
            write_service_time = store->stats.write_service_time / store->stats.nreqs;
        }
        ...
        RDMA_LOG(INFO) << "memstore[" << store->thread_id_ << "]: req=" << diff.nreqs
                       << " res=" << diff.nresponses << " r=" << diff.nreads
                       << " asize=" << asize << " csize=" << csize
                       << " ra=" << diff.nreadsagain << " w=" << diff.nwrites
                       << " wa=" << diff.nwritesagain << " g=" << diff.ngets
                       << " p=" << diff.nputs << " range=" << diff.nranges
                       << " gh=" << diff.nget_hits << " pl=" << diff.nput_lc
                       << " gl=" << diff.nget_lc << " glh=" << diff.nget_lc_hits
                       << " rg=" << diff.nget_rdma << " rgs=" << diff.nget_rdma_stale
                       << " rgi=" << diff.nget_rdma_invalid
                       << " rig=" << diff.ngetindex_rdma
                       << " rigd=" << diff.ngetindex_rdma_indirect
                       << " rigi=" << diff.ngetindex_rdma_invalid
                       << " st=" << service_time << " rst=" << read_service_time
    ...
    void new_conn_handler(int fd, short which, void *arg) {
        ...
        for (uint32_t i = 0; i < store->conn_queue.size(); i++) {
            ...
            RDMA_ASSERT(client_fd < NOVA_MAX_CONN)
                << "memstore[" << store->thread_id_ << "]: too large " << client_fd;
            nova_conns[client_fd] = conn;
            RDMA_LOG(DEBUG) << "memstore[" << store->thread_id_ << "]: connected "
                            << client_fd;
            RDMA_ASSERT(event_assign(&conn->event, store->base, client_fd,
                                     EV_READ | EV_PERSIST, event_handler, conn) == 0)
                << fd;
    ...
    void NovaConnWorker::Start() {
        ...
        if ((f & EV_FEATURE_O1)) {
            RDMA_LOG(DEBUG) << "O(1) event notification is supported.";
        }
        ...
        /* Timer event for new connection */
        {
            ...
            RDMA_ASSERT(event_assign(&new_conn_timer_event, base, -1, EV_PERSIST,
        ...
        }
        {
            ...
            RDMA_ASSERT(event_assign(&async_cq_->readevent, base, async_cq_->read_fd,
                                     EV_READ | EV_PERSIST, async_complete_event_handler,
                                     this) == 0)
                << async_cq_->read_fd;
            RDMA_ASSERT(event_add(&async_cq_->readevent, 0) == 0) << async_cq_->read_fd;
        }
    ...
    void Connection::UpdateEventFlags(int new_flags) {
        ...
        RDMA_ASSERT(event_assign(&event, ((NovaConnWorker *) worker)->base, fd,
                                 new_flags, event_handler, this) == 0)
            << fd;
