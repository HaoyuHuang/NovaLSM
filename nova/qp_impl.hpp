...
//                printf("poll-%d, wr-%d\n", poll_result, wcs[i].wr_id);
                RDMA_LOG_IF(WARNING, wcs[i].status != IBV_WC_SUCCESS) <<
                                                                      "poll till completion error: "
                                                                      << wcs[i].status
                                                                      << " "
                                                                      << ibv_wc_status_str(
    ...
    public:
        ...
        static bool ready2rcv(ibv_qp *qp, QPAttr &attr, RNicHandler *rnic,
            ...
            RDMA_ASSERT(rc == 0) << "ready to rcv failed " << rc;
            return rc == 0;
    ...
    public:
        ...
        init(ibv_qp *&qp, ibv_cq *&cq, ibv_cq *&recv_cq, RNicHandler *rnic) {
            ...
            RDMA_ASSERT(config.max_send_size == RC_MAX_SEND_SIZE);
            RDMA_ASSERT(config.max_recv_size == RC_MAX_RECV_SIZE);
            ...
            if ((cq = ibv_create_cq(rnic->ctx, RC_MAX_SEND_SIZE, nullptr,
                                    nullptr, 0)) == nullptr) {
                RDMA_LOG(ERROR) << "create send cq for UD QP error: "
                                << strerror(errno);
                return;
            }
            ...
            if ((recv_cq = ibv_create_cq(rnic->ctx, RC_MAX_RECV_SIZE, nullptr,
                                         nullptr, 0)) == nullptr) {
                RDMA_LOG(ERROR) << "create recv cq for UD QP error: "
                                << strerror(errno);
                return;
            }
            ...
            if ((qp = ibv_create_qp(rnic->pd, &qp_init_attr)) == nullptr) {
                RDMA_LOG(ERROR) << "create send qp for UD QP error: "
                                << strerror(errno);
                return;
            }
...
//            ready2init(qp, rnic, config); // shall always succeed
            ...
            RDMA_LOG(DEBUG) << "init qp portid: " << rnic->port_id;
            ...
            flags = IBV_QP_STATE;
            qp_attr.qp_state = IBV_QPS_RTR;
            RDMA_LOG(DEBUG) << "rtr qp portid: " << rnic->port_id << " lid:"
                            << rnic->lid;
            rc = ibv_modify_qp(qp, &qp_attr, flags);
            RDMA_ASSERT(rc == 0) << rc;
...
//            memset(&qp_attr, 0, sizeof(struct ibv_qp_attr));
        ...
        static ibv_ah *create_ah(RNicHandler *rnic, QPAttr &attr) {
            ...
            RDMA_LOG(DEBUG) << "ah: " << ah_attr.dlid << ":" << ah_attr.port_num
                            << ":" << rnic->port_id;
