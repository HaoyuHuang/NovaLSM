...
// "*dest" must remain live while this Writer is in use.
        RDMALogWriter::RDMALogWriter(nova::NovaRDMAStore *store,
            ...
            write_result_ = new WriteState[nova::NovaDCConfig::dc_config->dc_servers.size()];
            for (int i = 0;
                 i < nova::NovaDCConfig::dc_config->dc_servers.size(); i++) {
        ...
        void RDMALogWriter::Init(const std::string &log_file_name) {
            ...
            if (it == logfile_last_buf_.end()) {
                LogFileBuf *buf = new LogFileBuf[nova::NovaDCConfig::dc_config->dc_servers.size()];
                logfile_last_buf_.insert(std::make_pair(log_file_name, buf));
                for (int i = 0;
                     i <
                     nova::NovaDCConfig::dc_config->dc_servers.size(); i++) {
                    buf[i] = {
                            .base = 0,
                            .offset = 0,
                            .size = 0
...
//            int server_id = nova::NovaCCConfig::config->my_server_id;
//            char *buf = mem_manager_->ItemAlloc(slabclassid);
//            Init(log_file);
//            logfile_last_buf_[log_file][server_id] = {
//                    .base = (uint64_t) buf,
//                    .offset = 0,
//                    .size = nova::NovaCCConfig::config->log_buf_size
//            };
...
//            nova::Fragment *frag = nova::NovaCCConfig::config->db_fragment[db_index];
//
...
//                if (remote_server_id ==
//                    nova::NovaCCConfig::config->my_server_id) {
//                    continue;
//                }
//                nreplicas++;
//
...
//                    write_result_[remote_server_id].rdma_wr_id = store_->PostSend(
//                            nullptr, 1 + 4 + log_file_name.size(),
//                            remote_server_id, 0);
//
//                } else {
//                    // WRITE.
//                    write_result_[remote_server_id].rdma_wr_id = store_->PostWrite(
//                            buf, slice.size(), remote_server_id,
...
//
//                    if (n % 1000000 == 0) {
//                        if (write_result_[remote_server_id].result !=
...
//                                                    << remote_server_id << " w:"
//                                                    << write_result_str(
//                                                            write_result_[remote_server_id].result);
//                        }
//                    }
//
//
