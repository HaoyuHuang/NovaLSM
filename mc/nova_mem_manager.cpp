...
Slab::Slab(char *buf) { next_ = base; }

void Slab::Init(uint32_t item_size) {
  uint64_t size = NovaConfig::config->max_msg_size;
...
void SlabClass::FreeItem(char *buf) { free_list.push_back(buf); }

void SlabClass::AddSlab(Slab *slab) { slabs.push_back(slab); }

NovaPartitionedMemManager::NovaPartitionedMemManager(char *buf,
                                                     uint64_t data_size) {
  char *data_buf = buf;
  uint64_t slab_size =
      NovaConfig::config->max_msg_size;  // SLAB_SIZE_MB * 1024 * 1024;
  uint64_t size = NovaConfig::config->max_msg_size;
  ...
  for (uint64_t i = 0; i < ndataslabs; i++) {
...
uint32_t NovaPartitionedMemManager::slabclassid(uint32_t size) {
  uint32_t res = 0;
  if (size == 0 || size > NovaConfig::config->max_msg_size) return 0;
...
NovaMemManager::NovaMemManager(char *buf) {
  uint64_t partition_size = NovaConfig::config->cache_size_gb * 1024 * 1024 *
                            1024 / NOVA_MEM_PARTITIONS;
  char *base = buf;
...
char *NovaMemManager::ItemAlloc(uint64_t key, uint32_t scid) {
  return partitioned_mem_managers_[key % partitioned_mem_managers_.size()]
      ->ItemAlloc(scid);
}
...
void NovaMemManager::FreeItem(uint64_t key, char *buf, uint32_t scid) {
  partitioned_mem_managers_[key % partitioned_mem_managers_.size()]->FreeItem(
      buf, scid);
}

void NovaMemManager::FreeItems(uint64_t key, const std::vector<char *> &items,
                               uint32_t scid) {
  partitioned_mem_managers_[key % partitioned_mem_managers_.size()]->FreeItems(
      items, scid);
}
}  // namespace nova